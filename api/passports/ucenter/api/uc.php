...
if(!defined('IN_UC')) {
	...
	if(in_array($get['action'], array('test', 'deleteuser', 'renameuser', 'gettag', 'synlogin', 'synlogout', 'updatepw', 'updatebadwords', 'updatehosts', 'updateapps', 'updateclient', 'updatecredit', 'getcreditsettings', 'updatecreditsettings'))) {
		require_once DISCUZ_ROOT.'../../../libraries/common.inc.php';
		require_once DISCUZ_ROOT.'../../../libraries/db_mysql.inc.php';
		$GLOBALS['db'] = new DB_Sql;
		$GLOBALS['db']->connect($dbname,$dbhost,$dbuser,$dbpasswd);
		$GLOBALS['tablepre'] = $tb_prefix;
...
class uc_note {
	...
	function synlogin($get, $post) {
		...
		$sql = "SELECT * FROM ".$this->tablepre."members WHERE username='$username'";
		$user_info = $pdb->GetRow($sql);
		if($user_info) {
			//update login times
			$loginip = pb_get_client_ip();
			$pdb->Execute("UPDATE {$this->tablepre}members SET last_login=".time().",last_ip='".$loginip."' WHERE id='{$user_info['id']}'");
	...
	function synlogout($get, $post) {
		...
		header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
		unset($_SESSION["MemberID"],$_SESSION["MemberName"]);
		//logout from phpb2b since 2011.1.11
		uclearcookies();
	}
	...
	function updatepw($get, $post) {
		...
		if($password){
			$newpw = md5($password);
			//$pdb->Execute($sql = "UPDATE {$this->tablepre}members SET userpass='{$newpw}' WHERE username='{$username}'");
			return API_RETURN_SUCCEED;
		}else{
			return API_RETURN_FAILED;
		}
