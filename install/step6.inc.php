...
<?php if($db_error){ ?>
<span class="error"><?php echo $error_info."<br>".$_db_error_and_back;?></span>
<div><input type="button" onclick="javascript:history.go(-1);" value="<?php echo $_go_back;?> : <?php echo $steps[--$step];?>" class="btn" /></div>
<?php }else{ ?>
...
<input type="button" id="btn_goback" onclick="javascript:history.go(-1);" value="<?php echo $_go_back;?> : <?php echo $steps[--$step];?>" class="btn" />
<input type="button" onClick="$('#install').submit();$('#btn_installnow,#btn_goback').attr('disabled',true);" id="btn_installnow" class="btn" value="<?php echo $_next_step;?> : <?php echo $_start_install;?>" />
<?php } ?>
</div>
</div>
</body>
