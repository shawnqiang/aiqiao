...
class Attachment extends PbController {
	...
	function Attachment($user_file = '')
	{
		...
		if (isset($_PB_CACHE['setting']['thumb_save_orignal'])) {
			$this->if_orignal = $_PB_CACHE['setting']['thumb_save_orignal'];
		}
		if (!empty($_PB_CACHE['setting']['thumb_small'])) {
			$this->small_scale = $_PB_CACHE['setting']['thumb_small'];
		}
		if (!empty($_PB_CACHE['setting']['thumb_middle'])) {
			$this->middle_scale = $_PB_CACHE['setting']['thumb_middle'];
		}
		if (!empty($_PB_CACHE['setting']['thumb_large'])) {
			$this->large_scale = $_PB_CACHE['setting']['thumb_large'];
			$this->if_thumb_large = true;
		}
		if (!empty($_PB_CACHE['setting']['waterface']) && file_exists(DATA_PATH. "fonts".DS.$_PB_CACHE['setting']['waterface'])) {
			$this->fontFace = $_PB_CACHE['setting']['waterface'];
		}
 	...
 	function upload_process($type_id = '0')
 	{
		...
		$this->upload_url = str_replace(array(DS, "\\", "\'"), "/", $this->upload_dir).'/';
 		if (isset($_FILES) && $_FILES[$this->upload_form_field]['size']>0) {
 			...
 			$file_ext = fileext($_FILES[$this->upload_form_field]['name']);
 			if ($this->is_image) {
 				//check image format
	        ...
	        if ($this->is_image) {
		        ...
		        if($this->if_watermark){
		        	...
		        	$markimg->fontFile = PHPB2B_ROOT."data/fonts/".$this->fontFace;
		        	if($this->is_water_image){
		        		$markimg->appendImageMark(PHPB2B_ROOT.'images/watermark.png');
		        	}else{
