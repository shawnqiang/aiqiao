...
if (isset($_POST['saveregisterfile'])) {
	$updated = false;
	if (isset($_PB_CACHE['setting']['register_type']) && $_PB_CACHE['setting']['register_type']!="close_register") {
		...
		if (!empty($_POST['data']['setting']['post_filename']) && !pb_strcomp($_POST['data']['setting']['post_filename'],$_POST['data']['post_filename'])) {
		    $renameResult = rename(PHPB2B_ROOT. 'offer/post.php', PHPB2B_ROOT. 'offer/'.$_POST['data']['setting']['post_filename']);
		}		
	}
	$updated = $setting->replace($_POST['data']['setting']);
	if($updated){
